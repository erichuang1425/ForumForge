...
| --- | --- | --- | --- | --- | --- |
| 2026-07-16 | Extension source `bf24646`, version `0.1.0` | `forumforge-0.1.0-chrome.zip`; SHA-256 `63248e829b2d5baba7848e4c430d22d8579cfdcaf5360317725931f5e493d5ab` | Package twice from the same clean source; compare hashes; inspect the ZIP entry allowlist, manifest permissions, source-map references, and bundled network/eval/storage-sync API patterns | Pass for deterministic structure only | Both hashes matched. The ZIP contained the five runtime files plus `icons/icon-{16,32,48,128}.png`; permissions were exactly `activeTab`, `scripting`, `sidePanel`, and `storage`. No host permissions, optional permissions, declared content scripts, source-map references, or scanned runtime API patterns were present. The clean extraction is retained under the ignored `tmp/` workspace for browser testing. Chrome control was unavailable, so the ZIP was not loaded as an extension. |
| 2026-07-16 | Reader source `451e023`, version `0.1.0` | Deterministic eight-story gallery in the isolated in-app browser; browser version unavailable; Microsoft Windows 10.0.26200 x64 | All six layouts at the default 1280 x 720 viewport; Q&A at 500 x 900 and 360 x 800; launcher-only states at desktop and 360 x 800; accepted-answer focus state | Pass for component-level visual layout only | The real reader component rendered Nairaland, Hacker News, PTT, 4chan, Arca, DC Inside, FMKorea, and Stack Overflow synthetic stories with the intended grouping and textual semantic cues. The 500- and 360-pixel checks had zero page-level horizontal overflow, and no runtime console errors were observed. This was not packaged-extension evidence; controls inside the closed shadow root were not targetable through that browser surface, so launcher interaction, keyboard/focus behavior, reduced-motion system behavior, 200% zoom, persistence, extension permissions, and network-panel checks remain pending. |
